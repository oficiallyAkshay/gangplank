#!/bin/bash
# on_failure / on_recovery command for gangplank's action.yml. Pushes one
...
if [ "${GANGPLANK_EXIT_CODE:-1}" = "0" ]; then
  ...
  title="Deploy failed (exit ${GANGPLANK_EXIT_CODE:-?})"
  body="${GANGPLANK_REASON:-see the run log} — ${GANGPLANK_RUN_URL:-unknown run}"
  priority="high"
fi
